...
端口说明:
  ...
  - 9999      : 密钥协商 + 数据传输端口（发送端连接接收端的9999端口）

数据流向:
  发送端抓包 → AES加密+HMAC+RSA签名 → TCP:9999 → 接收端解密验证
  数据文件: latest_capture.json(抓包) / latest_received.json(接收)
```
...
1. 在发送端 Web 界面，输入接收端的 IP 和协商端口：
   - **目标 IP**: `192.168.157.207`（接收端 VM 的 IP）
